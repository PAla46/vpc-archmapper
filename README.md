...
- **`*-diagram.html`** — open in any browser for a full-window **interactive**
  architecture diagram. **Drag to pan**, **mouse wheel / touch to zoom**, and
  use the +/−/reset buttons in the slim top toolbar. A small pill on the page
  links to the raw `.mmd` file for importing into Excalidraw / Mermaid Live
  Editor.
